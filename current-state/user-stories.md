...
This document holds the user stories for the Local
Produce Exchange. It is the companion to the team's use cases. 

Each story names a user, the goal that user wants, and the benefit they get,
followed by acceptance criteria written as Gherkin scenarios. The format follows
our team template.

Every story carries a "Source use case(s)" line so each one is traceable back to
the current-scope use cases, which are in turn traceable back to the requirements
and the charter. This is the "Traceable" quality the template asks for.

Each story is one distinct functionality. Edge cases and permission rules are
captured as scenarios inside the relevant story, not as separate stories, per the
requirements.
...
**Notes / open questions / assumptions**
- Suspending an account is an admin feature held back from this slice, but the
  suspended state is still checked at login.
...
**Notes / open questions / assumptions**
- ASSUMPTION: the source documents require invite-only registration but do not
  say who issues invite tokens. Confirm whether any active member can invite, or
  only an admin. If the team does not decide the issuer, keep US-01 as core and
  treat US-04 as optional or pending.
...
**Notes / open questions / assumptions**
- Active listings are seeded for this slice; creating listings is out of the
  current scope.
...
can decide whether to request it.
...
- Then the system shows its full details, including the quantity available, the
  tags, and the pickup window
...
**Story:** As a recipient, I want to submit a request for a specified quantity
from a listing so that my request joins the listing's queue and is handled in
order.
...
**Notes / open questions / assumptions**
- This story models the requirements Sample User Story #1, adapted to the
  request-queue scope.
...
**Story:** As a poster, I want to see the pending requests on my listing in the
order they were received so that I can handle them fairly and in order.
...
- Then the system shows the pending requests oldest first, with the quantity
  requested and the quantity remaining
...
**Story:** As a poster, I want to approve or deny a pending request on my listing
so that I control who receives my items, in order, without conflicts.
...
- Given the poster owns the listing
- And approving this request would push the approved quantity past the remaining
  quantity
...
**Notes / open questions / assumptions**
- DENIED is terminal.
- ASSUMPTION: the poster handles requests in the order received. Confirm this
  with the team.
- Approve and deny are two outcomes of one decision, so they are scenarios in
  this one story, not separate stories.
...
**Story:** As a recipient, I want to withdraw a pending request I no longer need
so that it leaves the queue and the poster knows.
...
**Story:** As a member who is the poster or the recipient for an exchange, I want
to send and read messages in that exchange's private thread so that the two of us
can coordinate the exchange.
...
This table is the authoritative story list for the current scope. It traces each
story back to its source use case and forward to the requirement or in-scope
deliverable it satisfies. This gives two-way traceability: requirements and
in-scope deliverables -> current-scope use cases -> user stories.
...
### 3.1 Use-case coverage check

Every current-scope use case maps to exactly one story. UC-01 through UC-12 are
all covered. The database schema is a technical deliverable, not an actor goal,
so it has no story; it is traced in Section 4.

### 3.2 Role coverage check

Every current-scope role is the actor of at least one story:

| Role | Stories where this role is the actor |
| --- | --- |
| Guest | US-01 |
| Member | US-02, US-03, US-04, US-05, US-06, US-07, US-12 |
| Poster | US-09, US-10 |
| Recipient | US-08, US-11 |

### 3.3 Business-rule coverage check

| Business rule (current scope) | Covered by scenario |
| --- | --- |
| Request cannot exceed total quantity available | US-08 Scenario 2 |
| Request quantity must be positive | US-08 Scenario 3 |
| No duplicate open request on the same listing | US-08 Scenario 4 |
| Requests are shown and handled in the order received | US-09 Scenario 1; US-10 |
| Approved quantity never exceeds remaining quantity (conflict prevention) | US-10 Scenario 3 |
| REQUESTED -> APPROVED or DENIED; DENIED terminal | US-10 Scenarios 1, 2 |
| Withdraw only from REQUESTED; CANCELLED terminal | US-11 Scenarios 1, 2 |
| Non-party cannot read or post in a thread | US-12 Scenario 3 |

## 4. Non-story technical deliverable

The database schema is in scope now but is a technical deliverable, not an actor
goal, so it has no user story. It supports every story above and is traced here so
reviewers can confirm nothing was dropped.
...
| Reviews data stored in the schema | Reviews are stored as data now; leaving and viewing reviews is later scope, not a current story |

## 5. Assumptions and open questions

These items are not yet approved by the team. Each one should be confirmed or
changed before the requirements are locked. They are also noted inline in the
affected stories.

| ID | Item | Affected stories | Status |
| --- | --- | --- | --- |
| A1 | Who may issue invite tokens (any active member, or admin only) | US-04 | Open |
| A2 | The poster handles requests in the order received | US-10 | Assumed |
| A3 | Do invite tokens expire, and after how long | US-01 | Open |
| A4 | The exact filter set for browse and search | US-06 | Open |

## 6. INVEST and Definition of Ready note

Each story above was checked against the INVEST criteria. Each one is independent
enough to build on its own, states clear value in its "so that" clause, is small
enough for a single sprint, and has testable acceptance criteria. Stories with
open assumptions in Section 5 are not yet "Ready" in the Definition of Ready sense
