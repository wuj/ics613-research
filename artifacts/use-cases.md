...
- Goal: Leave a short rating and review of the other party after an exchange is completed, so each party gets feedback on how the exchange went.  
...
- Main success flow:  
  ...
  3. The system shows these grouped by status, with links to act on each item.  
- Alternate and exception flows:  
